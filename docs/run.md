...
pnpm -F @gaskya/main-backend dev
pnpm -F @gaskya/web dev
pnpm -F @gaskya/admin-web dev
pnpm -F @gaskya/website dev
...
```bash
pnpm -F @gaskya/main-backend dev        # 8081
pnpm -F @gaskya/web dev                 # 5173 → calls main-backend
pnpm -F @gaskya/website dev             # 3000 → links to web
```
...
```bash
# One app
pnpm -F @gaskya/main-backend build
pnpm -F @gaskya/web build
...
```bash
# Express service
pnpm -F @gaskya/main-backend build && pnpm -F @gaskya/main-backend start

# Vite preview (production bundle on a preview server, port 4173/4174)
pnpm -F @gaskya/web build && pnpm -F @gaskya/web start
pnpm -F @gaskya/admin-web build && pnpm -F @gaskya/admin-web start

# Next.js production
pnpm -F @gaskya/website build && pnpm -F @gaskya/website start
```
...
```bash
# Single project
pnpm -F @gaskya/main-backend typecheck
pnpm -F @gaskya/web lint
...
- `Module not found: '@gaskya/ui'` after a rename — restart the dev server; tsconfig path edits aren't watched.
